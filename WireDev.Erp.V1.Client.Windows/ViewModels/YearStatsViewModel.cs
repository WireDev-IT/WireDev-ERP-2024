...
{
    internal class YearStatsViewModel : TimeStatsViewModel
    {
        protected new List<YearStats>? _statsList = null;
        public new List<YearStats>? StatsList
        {
            get => _statsList;
            set
            {
        ...
        public override YearStats? SelectedTime => StatsList != null && SelectedTimeStatIndex < StatsList.Count ? StatsList[SelectedTimeStatIndex] : null;
        ...
        protected override async Task<bool> SetStatDataAsync(TimeStatQuery time)
        {
            ...
            {
                if (await GetStatDataAsync(time, LoadingStatCts.Token) is YearStats newData)
                {
                    LoadingStatCts.Token.ThrowIfCancellationRequested();
                    YearStats? existing = StatsList.Find(x => x.Date == newData.Date);
                    if (existing == null)
                    {
